...
def mayor(x: Nat, y: Nat) -> bool:
    if es_cero(x) and es_cero(y):
        return False
    elif es_cero(x) and not es_cero(y):
        return False
    elif not es_cero(x) and es_cero(y):
        return True
    else:
...
def menor_igual(x: Nat, y: Nat) -> bool:
    if es_cero(x) and es_cero(y):
        ...
        return True
    elif es_cero(y) and not es_cero(x):
        return False
    else:
...
def resta(x: Nat, y: Nat) -> Nat:
    if es_cero(y):
        ...
        raise ValueError('No se puede restar un número mayor a otro')
    else:
